defmodule Apa.Workers.LogsProducer do
  @moduledoc """
  (modified) Logs producer for the publish/subscribe tutorial step

  https://www.rabbitmq.com/tutorials/tutorial-three-elixir.html

  Produces logs in the form
  ...
  def init(config) do
    Logger.notice("Initializing LogsProducer")
  ...
  def terminate(reason, %LogsProducer{connection: connection} = _state) do
  ...
  defp next_step(%LogsProducer{interval: interval} = state) do
  ...
  defp setup_amqp(%LogsProducer{user: user, password: password, exchange: exchange} = state) do
    {:ok, connection} =
    ...
    Logger.debug("Declaring random queue")
  ...
  # Produce next value
  defp produce_value(%LogsProducer{current_value: current_value} = state) do
    ...
    {payload, %LogsProducer{state | current_value: new_value}}
  end

  defp send_value(value, %LogsProducer{queue: _queue, channel: channel, exchange: exchange} = state) do
    ...
    AMQP.Basic.publish(channel, exchange, "", value)
    {:ok, state}
  end
  ...
  defp parse_settings(%LogsProducer{} = partial_producer, %{"settings" => settings} = _config) do
    %LogsProducer{
      partial_producer
